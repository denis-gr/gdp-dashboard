...
st.title("График выражения")
expression = st.text_input("Введите выражение:", "5*x+3*sin(x)")
...
try:
    ...
    st.error(f"Ошибка в выражении: {e}")
    simplified_expr = ""
st.latex(f"Упрощенние: {latex(simplified_expr)}")
st.text(simplified_expr)
...
try:
    ...
    st.error(f"Ошибка в выражении: {e}")
    roots = ""
st.latex(f"Корни: {latex(roots)}")
st.text(roots)

try:
    droots = dsolve(Eq(expr, 0), parse_expr("y(x)"))
except Exception as e:
    st.error(f"Ошибка в выражении: {e}")
    droots = ""
st.latex(f"Корни2: {latex(droots)}")
st.text(droots)

try:
    lroots = laplace_transform(expr, "x", "s")[0]
except Exception as e:
    st.error(f"Ошибка в выражении: {e}")
    lroots = ""
st.latex(f"Лаплас: {latex(lroots)}")
st.text(lroots)

try:
    ilroots = inverse_laplace_transform(expr, "s", "x")
except Exception as e:
    st.error(f"Ошибка в выражении: {e}")
    ilroots = ""
st.latex(f"НеЛаплас: {latex(ilroots)}")
st.text(ilroots)


start = st.number_input("Начало интервала:", value=-10)
end = st.number_input("Конец интервала:", value=10)
step = st.number_input("Точек:", value=400)

exp2 = st.text_input("Введите выражение2:", "")
try:
    expr2 = parse_expr(exp2)
except Exception as e:
    exp2 = None
exp3 = st.text_input("Введите выражение3:", "")
try:
    expr3 = parse_expr(exp3)
except Exception as e:
    exp3 = None
...
y = lambdify("x", expr)(x)
fig = go.Figure()
fig.add_trace(go.Scatter(x=x, y=y, mode='lines', name='График'))
if exp2:
    y2 = lambdify("x", expr2)(x)
    fig.add_trace(go.Scatter(x=x, y=y2, mode='lines', name='График2'))
if exp3:
    y3 = lambdify("x", expr3)(x)
    fig.add_trace(go.Scatter(x=x, y=y3, mode='lines', name='График3'))
st.plotly_chart(fig)
