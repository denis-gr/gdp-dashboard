...
import numpy as np
from sympy import parse_expr, lambdify, simplify, solve, series, dsolve, Eq, latex, laplace_transform, inverse_laplace_transform, Integral, integrate, summation, limit, Limit, Sum


st.title("Решение с помощью преобзования Резольверты")
expression = str(st.text_input("Введите выражение:", "exp(x) - Integral(exp(x - t) * f(t), (t, 0, x)) - f(x)"))
...
st.markdown("##### Ядра")
def find_integral(expr):
    if type(expr) == Integral:
        return expr
    for arg in expr.args:
        if find_integral(arg):
            return find_integral(arg)
    return None

K = find_integral(expr).args[0].subs(parse_expr("f(t)"), 1)
K_L = K
st.latex(latex(K))
st.code(K)
KLS = [K_L]
for i in range(5):
    K__l = K.subs(parse_expr("t"), parse_expr("s"))
    K__r = K_L.subs(parse_expr("x"), parse_expr("s"))
    K_L = integrate(K__l * K__r, (parse_expr("s"), parse_expr("t"), parse_expr("x"))).simplify()
    KLS.append(K_L)
    st.latex(latex(K_L))
    st.code(K_L)
    
KLS_L = KLS[4] * parse_expr("l**(n-1)")

st.markdown("##### Суммируем")
expression2 = st.text_input("Введите выражение:", KLS_L)
try:
    expr2 = parse_expr(expression2)
except Exception as e:
    st.error(f"Ошибка в выражении: {e}")
    expr2 = parse_expr("x")
st.latex(latex(expr2))
st.code(expr2)
summa = summation(expr2, (parse_expr("n"), 0, parse_expr("oo"))).simplify()
...
try:
    ...
    f_0 = parse_expr("1")
f_l = f_0
exx = parse_expr(expression).subs(parse_expr("f(x)"), "0")
for i in range(5):
    f_l = exx.subs(parse_expr("f(t)"), f_l.subs(parse_expr("x"), parse_expr("t"))).simplify()
    st.latex(latex(f_l))
    st.code(f_l)
    
st.markdown("##### Лимит")
expression3 = st.text_input("Введите выражение:", f_l)
try:
    expr3 = parse_expr(expression3)
except Exception as e:
    st.error(f"Ошибка в выражении: {e}")
    expr3 = parse_expr("x")
lim = Limit(expr3, parse_expr("n"), parse_expr("oo"))
st.latex(latex(lim))
st.code(lim)
lim = lim.simplify()
st.latex(latex(lim))
